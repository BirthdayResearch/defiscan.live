context('/oracles/TSLA-USD macbook-13', () => {
  ...
  it('should select button and reload graph when clicked', () => {
    ...
    cy.findAllByTestId('Oracles.GraphPeriodButton').eq(3).should('have.class', 'text-gray-900 dark:text-primaryDark-500 dark:border-gray-700 dark:hover:bg-primaryDark-500 dark:hover:text-white dark:bg-gray-900 bg-gray-200 border-gray-200 hover:bg-primary-50 hover:border-primary-50')
...
context('/oracles/TSLA-USD iphone-x', () => {
  ...
  context('should have <OracleTable>', () => {
    ...
    it('should select button and reload graph when clicked', () => {
      cy.findAllByTestId('Oracles.GraphPeriodButton').eq(2).click()
      cy.findAllByTestId('Oracles.GraphPeriodButton').eq(2).should('have.class', 'text-primary-500 bg-primary-100 border-primary-100 dark:bg-primaryDark-500 dark:border-primaryDark-500 dark:text-grayDark-900')
      cy.findAllByTestId('Oracles.GraphPeriodButton').eq(3).should('have.class', 'text-gray-900 dark:text-primaryDark-500 dark:border-gray-700 dark:hover:bg-primaryDark-500 dark:hover:text-white dark:bg-gray-900 bg-gray-200 border-gray-200 hover:bg-primary-50 hover:border-primary-50')
      cy.get('#oraclesGraphArea').should('not.exist')
      cy.findAllByTestId('Oracles.Spinner').should('exist')
      cy.get('#oraclesGraphArea').should('exist')
