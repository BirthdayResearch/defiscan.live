...
context("/transactions/[txid] on desktop - invalid txn id", () => {
  ...
  it("should not have view on metachain button", () => {
    cy.findByTestId("metascan-link-button").should("not.exist");
  });

  it("should have view on metachain button", () => {
    cy.visit(
      "/transactions/75d5ce3f6dbc1c57241df663cb7e9f363b7c0eb733b85df798221b0ea61bdfc8?network=MainNet",
    );
    cy.findByTestId("transaction-detail-block-height").click();
    cy.viewport("macbook-13");
