...
export function SearchResultTable(props: {
  ...
  if (props.searchResults === undefined) {
    return (
      <SearchStatusMessage message="Search by Block Hash, Block Height, Transaction ID, Vault ID or Address">
        <IoSearchSharp
          size={96}
          className="text-gray-400 opacity-30 dark:text-gray-100"
        />
      </SearchStatusMessage>
    );
  }
            ...
            >
              <div className="flex flex-row items-start gap-x-2">
                ...
                <div className="overflow-hidden">
                  <div className="overflow-hidden font-medium overflow-ellipsis dark:text-dark-gray-900">
                    {props.searchResults.title}
                  </div>
                  <div className="overflow-hidden text-sm text-gray-500 dark:text-gray-100">
                    {props.searchResults.type}
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="w-full rounded mt-1 bg-white dark:bg-gray-800 px-4 py-8">
      <div className="w-full flex flex-col bg-white dark:bg-gray-800 items-center">
        {props.children}
        <span className="text-center text-gray-400 dark:text-gray-100">
          {props.message}
        </span>
      </div>
    </div>
