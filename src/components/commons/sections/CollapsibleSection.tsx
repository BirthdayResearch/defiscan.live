      ...
      >
        ...
        >
          {(() => {
            if (isOpen) {
              return (
                <MdOutlineKeyboardArrowUp className="h-6 w-6 text-gray-900" />
              );
            }
            return (
              <MdOutlineKeyboardArrowDown className="h-6 w-6 text-gray-900" />
            );
          })()}
        </div>
      </div>
      <div
        className={classNames("mt-4", isOpen ? "block" : "hidden", {
          "md:block": props.mdNotCollapsible,
        })}
      >
        {props.children}
      </div>
