      ...
      <CollapsibleSection heading='Overview' mdNotCollapsible>
        <div className='flex flex-wrap' data-testid='SupplyStats.Desktop'>
          ...
          <div className='w-full lg:w-9/12 flex flex-wrap -m-1' data-testid='IndexStats.Desktop'>
            ...
            >
              <div className='mt-auto text-gray-500 text-sm dark:text-dark-400'>
                <span className='text-black font-medium mr-1 dark:text-white'>{CalculatePercentage(data.supply.total, data.supply.max)}</span>
                of max supply
              </div>
            </StatCard>
            <StatCard
            ...
            >
              <div className='mt-auto text-gray-500 text-sm dark:text-dark-400'>
                <span
                  className='text-black font-medium mr-1 dark:text-white'
            ...
            <StatCard
              ...
              testId='StatCard.Tvl'
            >
              <div className='mt-auto text-gray-500 dark:text-white flex divide-x text-sm -mx-1'>
                <div className='px-1'>
                  DEX:
                  <span
                    className='text-black font-medium ml-1 dark:text-dark-400'
            ...
            >
              <div className='mt-auto text-gray-500 text-sm dark:text-dark-400'>
                <span className='text-black mr-1 dark:text-white'>{CalculatePercentage(data.supply.burned, data.supply.total)}</span>
                of total minted
              </div>
            </StatCard>
          </div>
...
function StatCard (props: PropsWithChildren<{ infodesc: string, heading: string, stat: number | undefined, prefix?: string, suffix?: string, testId: string }>): JSX.Element {
  return (
    <div className='w-full md:w-1/2 p-1' data-testid={props.testId}>
      <div className='flex flex-col border dark:bg-dark-500 dark:border-dark-100 border-gray-200 p-4 md:p-5 rounded-lg'>
        <div className='flex items-center'>
          <span className='font-normal text-sm md:text-base mr-1 dark:text-white'>{props.heading}</span>
        ...
        <div className='flex flex-wrap items-center'>
          <ReactNumberFormat
            ...
            displayType='text'
            thousandSeparator
            className='text-lg lg:text-2xl font-semibold dark:text-dark-50'
            decimalScale={0}
            prefix={props.prefix !== undefined ? props.prefix : ''}
          />
          {props.suffix !== undefined && <span className='ml-1 dark:text-dark-400'>{props.suffix}</span>}
        </div>
        <div className='mt-2'>
          {props.children}
        </div>
