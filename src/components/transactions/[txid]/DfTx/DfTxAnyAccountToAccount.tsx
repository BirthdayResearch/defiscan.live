import { DfTx, AnyAccountToAccount } from '@defichain/jellyfish-transaction'
            ...
            return (
              <AdaptiveList key={`from-${scriptFromAddress}`} className='mb-1'>
                ...
                {scriptBalances.balances.map(balance => {
                  return (
                    <FromAmountRow balance={balance} key={`from-${balance.amount.toFixed(8)}-${balance.token}`} />
                  )
                })}
              </AdaptiveList>
            )
          ...
          {props.dftx.data.to.map(scriptBalances => {
            ...
            return (
              <AdaptiveList key={`to-${scriptToAddress}`} className='mb-1'>
                ...
                {scriptBalances.balances.map(balance => {
                  return (
                    <ToAmountRow balance={balance} key={`to-${balance.amount.toFixed(8)}-${balance.token}`} />
                  )
                })}
              </AdaptiveList>
            )
...
function FromAmountRow (props: {
  balance: TokenBalance
}): JSX.Element {
  return (
    <AdaptiveList.Row name='Amount'>
      <div className='flex flex-row'>
        <span data-testid='DfTxAnyAccountToAccount.fromAmount'>{props.balance.amount.toFixed(8)}</span>
        <TokenSymbol
          tokenId={props.balance.token} className='ml-1'
...
function ToAmountRow (props: {
  balance: TokenBalance
}): JSX.Element {
  return (
    <AdaptiveList.Row name='Amount'>
      <div className='flex flex-row'>
        <span data-testid='DfTxAnyAccountToAccount.toAmount'>{props.balance.amount.toFixed(8)}</span>
        <TokenSymbol
          tokenId={props.balance.token} className='ml-1'
