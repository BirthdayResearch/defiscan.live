...
export function PoolPairGraph (props: { poolpair: PoolPairData }): JSX.Element {
  ...
  const [feed, setFeed] = useState<PoolSwapAggregatedData[] | undefined>(undefined)
  ...
  return (
    <div className='bg-gray-50 rounded-lg py-4 md:py-6 px-4 md:px-8 flex flex-col' data-testid='PoolPairGraph'>
      <span className='block w-full font-medium text-lg' data-testid='Title'>Volume (24H)</span>
...
function VolumeBarChart ({ feed }: { feed: PoolSwapAggregatedData[] }): JSX.Element {
...
function TooltipDialog ({ payload }: TooltipProps<any, any>): JSX.Element | null {
  ...
  const blockTime = format(fromUnixTime(feed.block.medianTime), 'MMM dd, hh:mm aa')
...
async function fetchDailySwaps (api: WhaleApiClient, poolpairId: string): Promise<PoolSwapAggregatedData[]> {
  const aggregates: PoolSwapAggregatedData[] = []

  const response = await api.poolpairs.listPoolSwapAggregates(poolpairId, PoolSwapAggregatedInterval.ONE_HOUR, 24)
