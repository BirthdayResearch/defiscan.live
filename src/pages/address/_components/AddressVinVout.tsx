...
export function AddressVinVout(props: AddressVinVoutProps): JSX.Element {
  ...
  useEffect(() => {
    if (props.expanded) {
      void Promise.all([getVins(props.txid), getVouts(props.txid)]).then(
        ([vinsData, voutsData]) => {
          setVins(vinsData);
          setVouts(voutsData);
        },
  ...
  if (props.expanded && vins === undefined && vouts === undefined) {
    ...
    return (
      <TransactionDetails address={props.address} vins={vins} vouts={vouts} />
    );
  } else {
    return <></>;
  }
...
function TransactionDetails(props: {
  ...
  return (
    <td
      colSpan={4}
      className="px-4 md:px-6 pt-2 pb-4 lg:p-6 bg-gray-200 bg-opacity-50 dark:bg-opacity-80 dark:bg-gray-800"
    >
      <div className="w-full font-medium text-lg dark:text-dark-gray-900">
        Details
      </div>
...
function TransactionDetailsLeft(props: {
  address: string;
  vins: TransactionVin[];
  network: NetworkName;
}): JSX.Element {
...
function TransactionDetailsRight(props: {
  address: string;
  vouts: TransactionVout[];
  network: NetworkName;
  dftx: DfTx<any> | undefined;
}): JSX.Element {
  return (
    <div className="w-full lg:w-1/2">
      <div
        ...
        {props.vouts.map((vout, index) => {
          const decoded =
            vout.script !== undefined
              ? fromScriptHex(vout.script?.hex, props.network)
              : undefined;
          ...
          return (
            <TransactionVectorRow
              ...
              network={props.network}
              isAddressClickable={
                props.dftx === undefined && decoded?.address !== undefined
              }
            />
          );
        })}
