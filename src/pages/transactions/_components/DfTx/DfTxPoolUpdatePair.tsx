      ...
      <div className="mt-5 flex flex-col space-y-6 items-start lg:flex-row lg:space-x-8 lg:space-y-0">
        ...
        {props.dftx.data.customRewards.length > 0 && (
          <div className="w-full lg:w-1/2">
            <div className="-mb-4 lg:mb-2 lg:-mt-8 font-medium text-gray-800 opacity-80 dark:text-gray-100">
              Custom Rewards
            </div>
            <AdaptiveTable>
              ...
              {props.dftx.data.customRewards.map((tokenReward) => {
                return (
                  <PoolUpdatePairTableRow
                    tokenReward={tokenReward}
                    key={tokenReward.token}
                  />
                );
              })}
            </AdaptiveTable>
...
function PoolUpdatePairDetailsTable(props: {
  ...
  return (
    <AdaptiveList className="w-full lg:w-1/2">
      ...
      <AdaptiveList.Row
        name="Commission"
        testId="DfTxPoolUpdatePair.commission"
      >
        {props.commission}
      </AdaptiveList.Row>
      <AdaptiveList.Row
...
function PoolUpdatePairTableRow(props: {
  tokenReward: TokenBalance;
}): JSX.Element {
  return (
    <AdaptiveTable.Row>
      <AdaptiveTable.Cell title="TOKEN" className="align-middle">
        <div className="flex items-center">
          <TokenSymbol
            tokenId={props.tokenReward.token}
            testId="DfTxPoolUpdatePair.Token"
          />
        </div>
      </AdaptiveTable.Cell>
      <AdaptiveTable.Cell title="AMOUNT" className="align-middle">
        <span data-testid="DfTxPoolUpdatePair.Amount">
          {props.tokenReward.amount.toFixed(8)}
        </span>
      </AdaptiveTable.Cell>
    </AdaptiveTable.Row>
  );
