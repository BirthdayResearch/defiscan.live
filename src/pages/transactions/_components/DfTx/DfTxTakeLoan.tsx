        ...
        {props.dftx.data.tokenAmounts.map((tokenData, i) => (
          <AdaptiveList.Row name='Amount' key={`token-amounts-${i}`}>
            <div className='flex flex-row'>
              <span data-testid={`DfTxTakeLoan.toAmount-${i}`}>{tokenData.amount.toFixed(8)}</span>
              <TokenSymbol tokenId={tokenData.token} className='ml-1' testId={`DfTxTakeLoan.toSymbol-${i}`} />
            </div>
          </AdaptiveList.Row>
        ))}
      </AdaptiveList>
