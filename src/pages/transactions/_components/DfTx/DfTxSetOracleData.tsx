        ...
        {props.dftx.data.tokens.length > 0 &&
          <AdaptiveTable className='w-full lg:w-1/2'>
            ...
            {props.dftx.data.tokens.map(tokenPrice => {
              return (
                tokenPrice.prices.map(price => {
                  return (
                    <SetOracleDataTokenRow
                      token={tokenPrice.token} price={price}
...
function SetOracleDataDetails (props: { oracleId: string, timestamp: BigNumber }): JSX.Element {
  return (
  ...
  return (
    <AdaptiveTable.Row className='dark:text-gray-100'>
      ...
      <AdaptiveTable.Cell title='AMOUNT' className='align-middle'>
        <span data-testid='DfTxSetOracleData.Amount'>{props.price.amount.toFixed(8)}</span>
      </AdaptiveTable.Cell>
      <AdaptiveTable.Cell title='CURRENCY' className='align-middle'>
        <span data-testid='DfTxSetOracleData.Currency'>{props.price.currency}</span>
      </AdaptiveTable.Cell>
    </AdaptiveTable.Row>
  )
}
