...
export function DfTxPoolAddLiquidity(
  ...
  const shareAddress =
    props.dftx.data.shareAddress !== undefined
      ? fromScript(props.dftx.data.shareAddress, network)
      : undefined;
      ...
      <div className="mt-5 flex flex-col space-y-6 items-start">
          ...
          <AdaptiveList.Row name="Share Address">
            {(() => {
              if (shareAddress?.address !== undefined) {
                return (
                  <AddressLink
                    address={shareAddress.address}
                    testId="DfTxPoolAddLiquidity.ShareAddress"
                    className="break-all"
                  />
                );
              }
        ...
        <div className="w-full grid gap-2 grid-cols-1 lg:grid-cols-2">
          {props.dftx.data.from.map((scriptBalance) => {
            const fromAddress =
              fromScript(scriptBalance.script, network)?.address ?? "N/A";
            return (
              <AdaptiveList key={`from-${fromAddress}`}>
                <AdaptiveList.Row name="From">
                  <AddressLink
                    address={fromAddress}
                    testId="DfTxPoolAddLiquidity.FromAddress"
                    className="break-all"
                  />
                </AdaptiveList.Row>
...
function PoolAddLiquidityRow(props: {
  tokenId: number;
  amount: BigNumber;
}): JSX.Element {
  return (
      ...
      <div className="table-cell px-4 md:px-6 py-3">
        <div className="flex items-center dark:text-gray-400">
          <TokenSymbol
            ...
            testId={`DfTxPoolAddLiquidity.${props.tokenId}-Symbol`}
          />
        </div>
      </div>
      <div
