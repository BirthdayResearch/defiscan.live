    ...
    <>
      {(() => {
        if (props.target.name !== null) {
          return <Head title={`${props.target.name} Countdown`} />;
        }

        return <Head title={`Block #${props.target.height} Countdown`} />;
      })()}
      ...
      <Container className="pt-8 pb-56">
        ...
        <CountdownSection
          ...
          estimatedTargetTime={estimatedTargetTime}
        />
        <BlocksInfoSection
          current={currentHeight}
          remaining={props.target.height - currentHeight}
        />
      </Container>
    </>
  );
...
export async function getServerSideProps(
  ...
  async function getEventFromPrismic(): Promise<PrismicDocument | undefined> {
    ...
    return _.find(events.results, (event) => {
      return (
        (event.data.slug.toLowerCase() === query.toLowerCase() &&
          event.data.network.toLowerCase() === network.toLowerCase()) ||
        (event.data.height === Number(query) &&
          event.data.network.toLowerCase() === network.toLowerCase())
      );
    });
  }
