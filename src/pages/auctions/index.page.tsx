    ...
    <>
      ...
      <Container className="pt-12 pb-20">
        <div className="flex items-center">
          <h1
            className="text-2xl font-medium dark:text-dark-gray-900"
            data-testid="AuctionsPage.Heading"
          >
            Auctions
          </h1>
        </div>
              ...
              <OverflowTable>
                ...
                {vaults.items.map((vault) => {
                  return vault.batches.map((batch) => (
                    <Link
                      href={{
                        pathname: `/vaults/${vault.vaultId}/auctions/${batch.index}`,
                      }}
                      key={batch.index}
                    >
                      <a className="contents">
                        <AuctionsTableRow batch={batch} vault={vault} />
                      </a>
                    </Link>
                  ));
                })}
