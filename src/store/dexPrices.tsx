...
export const dexPrices = createSlice({
  ...
  initialState,
  reducers: {
    update: (state, action: PayloadAction<{ dexPrices: DexPricesProps, denomination: string }>) => {
      state.dexPrices = {
        ...state.dexPrices,
 ...
 */
export const dexPricesSelectorByDenomination = createSelector([(state: DexPricesState) => state.dexPrices, (_state: DexPricesState, denomination: string): string => denomination], (dexPrices, denomination) => {
  ...
  useEffect(() => {
    function fetch (): void {
      void api.poolpairs.listDexPrices(denomination).then(data => {
        dispatch(dexPrices.actions.update({ dexPrices: data.dexPrices, denomination }))
      })
    }
